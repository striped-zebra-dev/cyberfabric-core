...
use crate::error::{ClientError, ServiceGatewayError};
use crate::api::{ProxyContext, ProxyResponse};
...
pub trait DataPlaneService: Send + Sync {
    async fn proxy_request(
        &self,
        ctx: ProxyContext,
    ) -> Result<ProxyResponse, ServiceGatewayError>;
}
...
pub trait OagwClientApi: Send + Sync {
    ...
    /// * `alias` - Upstream alias (e.g., "openai", "anthropic")
    /// * `tenant_id` - Tenant ID for the request
    /// * `request` - HTTP request to initiate the WebSocket connection
