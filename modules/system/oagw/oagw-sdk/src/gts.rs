...
impl GtsId {
    ...
    /// Format a GTS identifier from a schema (with trailing `~`) and an instance.
    #[must_use]
...
mod tests {
    ...
    fn parse_upstream_gts() {
        ...
        assert_eq!(format!("{schema}~"), UPSTREAM_SCHEMA);
        assert_eq!(uuid, id);
    }
    ...
    fn parse_route_gts() {
        ...
        let (schema, uuid) = parse_resource_gts(&s).unwrap();
        assert_eq!(format!("{schema}~"), ROUTE_SCHEMA);
        assert_eq!(uuid, id);
    }
    ...
    fn format_upstream_round_trip() {
        let id = Uuid::parse_str("7c9e6679-7425-40de-944b-e07fc1f90ae7").unwrap();
        let s = format_upstream_gts(id);
        assert!(s.starts_with(UPSTREAM_SCHEMA));
    ...
    #[test]
    fn parse_plugin_gts() {
        let s = "gts.x.core.oagw.auth_plugin.v1~x.core.oagw.apikey.v1";
        let gts = GtsId::parse(s).unwrap();
        assert_eq!(gts.schema, "gts.x.core.oagw.auth_plugin.v1");
    ...
    fn reject_invalid_uuid_in_resource() {
        let s = GtsId::format(UPSTREAM_SCHEMA, "not_a_uuid");
        assert!(matches!(
