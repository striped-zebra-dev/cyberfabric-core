...
use oagw_sdk::service::{DataPlaneService, ErrorSource, ProxyContext};
...
pub struct SharedProcessClient {
    ...
    /// Default tenant ID for requests (can be overridden per-request in the future)
    tenant_id: Uuid,
}
...
impl SharedProcessClient {
    ...
    pub fn new(
        ...
        debug!(
            "Creating SharedProcessClient for tenant_id={}",
            tenant_id
        );
...
impl OagwClientApi for SharedProcessClient {
    async fn execute(&self, alias: &str, request: Request) -> Result<Response, ClientError> {
        ...
        let stream = proxy_response.body.map(|result: Result<Bytes, oagw_sdk::service::BoxError>| {
            result.map_err(|e| {
