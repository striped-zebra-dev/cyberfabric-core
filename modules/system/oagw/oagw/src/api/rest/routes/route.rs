...
pub(super) fn register(mut router: Router, openapi: &dyn OpenApiRegistry) -> Router {
    ...
    // GET /api/oagw/v1/routes/{id} — Get route
    router = OperationBuilder::get("/api/oagw/v1/routes/{id}")
    ...
    router = OperationBuilder::put("/api/oagw/v1/routes/{id}")
    ...
    router = OperationBuilder::delete("/api/oagw/v1/routes/{id}")
    ...
    // GET /api/oagw/v1/upstreams/{upstream_id}/routes — List routes by upstream
    router = OperationBuilder::get("/api/oagw/v1/upstreams/{upstream_id}/routes")
        ...
        .query_param_typed("$top", false, "Maximum number of results (default 50, max 100)", "integer")
