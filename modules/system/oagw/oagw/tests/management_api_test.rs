...
async fn get_upstream_by_gts_id() {
    ...
    let response = app
        .oneshot(
            Request::builder()
                .method(Method::GET)
                .uri(format!("/api/oagw/v1/upstreams/{gts_id}"))
                .header("x-tenant-id", tenant_id().to_string())
                .body(Body::empty())
                .unwrap(),
        )
...
async fn get_upstream_invalid_gts_returns_400() {
    ...
    let response = app
        .oneshot(
            Request::builder()
                .method(Method::GET)
                .uri("/api/oagw/v1/upstreams/not-a-gts-id")
                .header("x-tenant-id", tenant_id().to_string())
                .body(Body::empty())
                .unwrap(),
        )
...
async fn get_upstream_nonexistent_returns_404() {
    ...
    let response = app
        .oneshot(
            Request::builder()
                .method(Method::GET)
                .uri(format!("/api/oagw/v1/upstreams/{fake_id}"))
                .header("x-tenant-id", tenant_id().to_string())
                .body(Body::empty())
                .unwrap(),
        )
...
async fn list_upstreams_with_pagination() {
    ...
    let response = app
        .oneshot(
            Request::builder()
                .method(Method::GET)
                .uri("/api/oagw/v1/upstreams?$top=2&$skip=1")
